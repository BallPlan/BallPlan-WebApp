...
import { ArrowLeft, Plus, Trash2, ImagePlus, Save, Eye } from 'lucide-react';
import { getVenueById, addVenue, updateVenue, getCategories } from '../lib/agentData';
...
function ItemRows({ title, items, onChange, priceLabel = 'Price' }) {
  ...
  const add = () => onChange([...items, { id: `new-${Date.now()}`, name: '', image: '', price: '', desc: '' }]);
      ...
      <div className="mt-3 space-y-3">
        ...
        {items.map((item, i) => (
          <div key={item.id || i} className="rounded-xl border border-ink/10 p-3 dark:border-white/10">
            ...
            />
            {item.cartAddCount != null && (
              <p className="mt-2 text-[11px] text-ink/40 dark:text-white/40">Added to cart {item.cartAddCount} time{item.cartAddCount === 1 ? '' : 's'}</p>
            )}
            <button
              type="button"
              onClick={() => remove(i)}
...
export default function AgentVenueForm() {
  const { id } = useParams();
  const isEdit = !!id;
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      hiddenFees: venue.hiddenFees.filter((f) => f.label).map((f) => ({ label: f.label, amount: Number(f.amount) || 0 })),
    };
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div className="space-y-5">
        {isEdit && (
          <div className="flex items-center gap-2 rounded-2xl bg-white p-4 text-sm shadow-card dark:bg-[#1a1b20]">
            <Eye size={16} className="text-brand" />
            <span className="text-ink/70 dark:text-white/70">
              <strong className="text-ink dark:text-white">{venue.viewsCount ?? 0}</strong> total views ·{' '}
              posted {venue.createdAt ? new Date(venue.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : '—'}
            </span>
          </div>
        )}
