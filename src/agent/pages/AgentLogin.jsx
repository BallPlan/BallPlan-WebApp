...
export default function AgentLogin() {
  ...
  const { isAgent, loading: authLoading, signIn } = useAgentAuth();
  ...
  useEffect(() => {
    if (isAgent) navigate('/', { replace: true });
  }, [isAgent, navigate]);

  if (authLoading || isAgent) return null;
      ...
      >
        <div className="flex flex-col items-center text-center">
          ...
          <div className="mt-3 flex items-center gap-1.5 text-brand">
            <Store size={16} />
            <span className="text-xs font-bold uppercase tracking-wider">Agent Access</span>
          </div>
          <h1 className="mt-1 text-2xl font-extrabold text-ink">BallPlan Agent</h1>
          <p className="mt-1 text-sm text-ink/50">Sign in to manage the venues you've posted.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="mt-7 space-y-4">
          ...
          <p className="text-center text-xs text-ink/40">
            Agent accounts are created by BallPlan admin or support — contact them if you don't have one.
          </p>
        </form>
      </motion.div>
    </div>
